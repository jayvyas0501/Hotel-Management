...
import { roomData } from "../../Constant/index.js";
import { useAuth } from "../../context/AuthContext";
import { useNavigate } from "react-router-dom";
import apiRequest from "../../lib/axiosInstance";

export default function Double() {
  const room = roomData[1]; // Dynamic later with useParams
  ...
  // Mock room availability (replace with actual API check)
  const isRoomAvailable = Math.random() > 0.3; // 70% chance of being available for demo
  ...
  return (
    <div className="p-6 flex flex-col lg:flex-row gap-10 max-w-7xl mx-auto">
      ...
      <div className="lg:w-1/2 bg-gray-200 p-8 rounded-2xl shadow-xl space-y-6">
        <h2 className="text-3xl font-bold text-primary mb-2">{room.title}</h2>
        ...
        <form className="space-y-5 text-gray-700" onSubmit={handleSubmit}>
          <div>
            <label className="block mb-1 font-medium text-black">Check-in</label>
            <input
              type="date"
              value={checkIn}
          ...
          <div>
            <label className="block mb-1 font-medium text-black">Check-out</label>
            <input
              type="date"
              value={checkOut}
